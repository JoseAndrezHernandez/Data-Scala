...
// COMMAND ----------
  ...
  .agg(
    ...
    ,sum("numero_supervivientes").alias("Numero,supervivientes")
    ,(sum("numero_supervivientes") + sum("numero_fallecidos")) .alias("Total_pacientes")
  
    ,(sum("numero_fallecidos")/(sum("numero_supervivientes") + sum("numero_fallecidos"))).alias("Tasa_mortalidad_luego_del_tratamiento")
    ,((sum("numero_supervivientes"))/(sum("numero_supervivientes") + sum("numero_fallecidos"))).alias("Tasa_de_supervivencia_luego_del_tratamiento")
...
// COMMAND ----------
  ...
  .agg(
    ((countDistinct("numero_de_mortalidad"))/(countDistinct("numero_de_supervivencia") + countDistinct("numero_de_mortalidad"))*100).alias("tasa_de_mortalidad_")
    ,((countDistinct("numero_de_supervivencia"))/(countDistinct("numero_de_supervivencia") + countDistinct("numero_de_mortalidad"))*100).alias("tasa_supervivencia")
    ,countDistinct("id_paciente").alias("Cantidad_de_pacientes")
    

   
  )
  .where(
    col("tasa_supervivencia").isNotNull && col("tasa_de_mortalidad_").isNotNull
  )
...
// COMMAND ----------
  ...
  display(
    ...
    .agg(
      ((countDistinct("promedio_sobre_nuevos_eventos_tumorales")) / (countDistinct("id_paciente"))*100).alias("promedio_sobre_nuevos_tumores")
